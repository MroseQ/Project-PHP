...
?>
<div id="tabela_TTT" style="margin-left: 41%; font-size: 300%">
<?php
$scores = db_fetch_all($conn, "SELECT DISTINCT `uzytkownik`.`Nazwa_Użytkownika`,`wynik`.`punkty_zdobyte`
FROM `uzytkownik` INNER JOIN `wynik` ON `wynik`.`ID_Użytkownika`=`uzytkownik`.`ID_Użytkownika` WHERE `wynik`.`ID_gra` LIKE 1 GROUP BY `wynik`.`punkty_zdobyte` DESC");
        echo '
