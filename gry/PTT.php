...
<div id="gra" style="margin-left: 23%; margin-right:20%; border-width: 4px; border-style: solid; border-radius: 5px; background-color: #a1a5b0">
    ...
    if ($koniec==true){
    $wynik = $_GET["wynik"];   
    $sql="SELECT * FROM `wynik` WHERE `ID_Użytkownika` LIKE \"{$_SESSION["userid"]}\" AND `ID_gra` LIKE '2'";
    $czyistnieje = db_fetch_single($conn, $sql);
    if ($czyistnieje == null){
        if ($wynik != 0) {
        $sql = "INSERT INTO `wynik` (`ID_Użytkownika`,`ID_gra`,`punkty_zdobyte`) VALUES (\"{$_SESSION["userid"]}\",2,\"{$wynik}\")";
        db_insert($conn,$sql);
    }}else{
    ...
    ?> 
    <a style="margin-left: 47%; text-shadow: 1px 1px black; font-size: 20px; color: yellow"href="../wyniki/PTT_Wyniki.php">Wyniki</a>
    <p style="text-align: center;"><b> Wykonaj swój ruch </b></p>
    <p style="margin-left: 3%"> Wynik: <span id="wynik_ptt" style="text-align:right"><b>0</b></span> </p>
    <br>
...
<script src="ptt.js" charset="utf-8"></script> 